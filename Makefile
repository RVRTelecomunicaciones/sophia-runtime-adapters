...
chaos-render-rules:       ## Render Sloth from test specs into committed test rules
	mkdir -p /tmp/chaos-render-tmp
	sloth generate \
	  --slo-period-windows-path ops/slo/windows \
	  --default-slo-period 5m \
	  --input ops/slo/test/ \
	  --out /tmp/chaos-render-tmp/
	cat /tmp/chaos-render-tmp/*.yaml > ops/prometheus/generated/test-slo-rules.yaml
	rm -rf /tmp/chaos-render-tmp
...
chaos-render-rules-check: ## CI gate: idempotent render of test rules
	...
	cat /tmp/chaos-render-check/*.yaml > /tmp/chaos-render-check/combined.yaml
	diff -u ops/prometheus/generated/test-slo-rules.yaml /tmp/chaos-render-check/combined.yaml
